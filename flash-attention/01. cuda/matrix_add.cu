...
{
    EL_TYPE *A, *B, *OUT; // declare pointers for 3 matrices for host (CPU) memory 
    EL_TYPE *d_A, *d_B, *d_OUT; // for device (GPU) memory
    ...
    for (int i = 0; i < NUM_ROWS; i++)
    {
        ...
        {
            //ensures that the matrices are stored in a contiguous block of memory, following row-major order.
            
            // This line calculates the linear index for a two-dimensional matrix stored in a one-dimensional array using row-major order. In row-major storage, all elements of a row are stored contiguously in memory before moving to the next row.
    ...
    // allocate device memory GPU for each matrix
    // The cudaMalloc function is used to request a block of memory of size sizeof(EL_TYPE) * NUM_ROWS * NUM_COLS, which is enough to store all elements of the matrix. The result is stored in the pointer d_A, which will be used to access the matrix data on the device.
    ...
    int num_block_ROWS = (NUM_ROWS + ROWS_BLOCK_SIZE - 1) / ROWS_block_size; // ceil(NUM_ROWS / ROWS_block_size)
    int num_block_COLS = (NUM_COLS + COLS_BLOCK_SIZE - 1) / COLS_block_size; // ceil(NUM_COLS / COLS_block_size)
    ...
    dim3 grid_dim(num_block_COLS, num_block_ROWS);
    dim3 block_dim(ROWS_block_size, COLS_block_size);
