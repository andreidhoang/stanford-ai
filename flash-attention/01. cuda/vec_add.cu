...
void test_vector_add(int N)
{
    ...
    // allocate vectors on the host device
    ...
    CUDA_CHECK(cuda_Memcpy(d_A, A, sizeof(EL_TYPE) * N, cudaMemcpyHostToDevice));
    CUDA_CHECK(cuda_Memcpy(d_B, B, sizeof(EL_TYPE) * N, cudaMemcpyHostToDevice));
